...
async function fetchCardsLight(): Promise<any[]> {
  const { data, error } = await supabase
    .from("cards")
    .select("id,cardname,imageurl,accessplan,cardcategory,subCategory,subSubCategory,created_at")
...
async function fetchTemplatesLight(): Promise<any[]> {
  const { data, error } = await supabase
    .from("templetDesign")
    .select("id,title,img_url,accessplan,category,subCategory,subSubCategory,created_at")
    .order("created_at", { ascending: false });
  if (!error) return data ?? [];
