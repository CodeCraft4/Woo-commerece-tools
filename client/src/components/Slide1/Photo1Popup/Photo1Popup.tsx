...
const Photo1Popup = ({ onClose, activeIndex, isAdminEditor = false }: Photo1PopupProps) => {
  const {
    images1,
    setSelectedImage1,
    selectedImg1,
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      ...
      if (isAdminEditor) {
        ...
        setImages1((prev: any[]) => [...prev, { id: imageId, src: url, fileName: objectPath }]);
      } else {
        setImages1((prev: any[]) => [...prev, { id: imageId, src: url, fileName: objectPath }]);
  ...
  const handleDelete = async (id: any) => {
    const imgToDelete: any = images1.find((img: any) => img.id === id);
    ...
    setImages1((prev) => prev.filter((img: any) => img.id !== id));
  ...
  const applyFilter = (filterCss: string) => {
    setDraggableImages1((prev) =>
      prev.map((img) => (img.id === activeFilterImageId1 ? { ...img, filter: filterCss } : img))
    );
  };

  if (imageFilter1) {
    const selectedImage = draggableImages1.find((img) => img.id === activeFilterImageId1);
    const previewImg =
      ...
      >
        ...
        {images1.map(({ id, src }) => (
          <Box
            key={id}
            onClick={() => {
              setSelectedImage1((prev: any) => {
                const isSelected = prev.includes(id);
                if (!isSelected) setActiveFilterImageId1(id);
                setDraggableImages1((imgs: any[]) =>
                  imgs.map((img) =>
                    img.id === id
