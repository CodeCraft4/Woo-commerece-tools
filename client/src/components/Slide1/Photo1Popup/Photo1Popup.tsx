...
const Photo1Popup = ({ onClose, activeIndex }: Photo1PopupProps) => {
  const {
    images1,
    setSelectedImage1,
    selectedImg1,
  ...
  const handleDelete = async (id: any) => {
    const imgToDelete: any = images1.find((img) => img.id === id);
    ...
    try {
      ...
      setImages1((prev) => prev.filter((img) => img.id !== id));
  ...
  const fetchUserImages = async () => {
    ...
    try {
      ...
      if (data?.images && Array.isArray(data.images)) {
        ...
        setImages1(formattedImages);
      } else {
        console.log("No images found for this user.");
        setImages1([]); // clear if none
      }
    } catch (err) {
