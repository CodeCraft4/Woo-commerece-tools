...
interface Media1PopupProps {
  onClose: () => void;
  mediaType: "video" | "audio";
  activeIndex?: number;
}

const Media1Popup = ({ onClose, mediaType, activeIndex }: Media1PopupProps) => {
  ...
  const handleAudioUpload = async () => {
    ...
    try {
      for (const file of audio1) {
        ...
        setSelectedAudioUrl1(publicData.publicUrl);
      }
        ...
        <Box>
          ...
          {audio1 && (
            ...
            >
              {isVideo ? (
                <video
                  src={URL.createObjectURL(audio1[0])}
                  controls
                  autoPlay={false}
                  onLoadedMetadata={(e) =>
                    setDuration1(e.currentTarget.duration)
                  }
              ...
              >
                ...
                >
                  <InfoOutline /> {(audio1[0].size / (1024 * 1024)).toFixed(0)}{" "}
                  MB
                </Typography>
                <Typography
                  sx={{ display: "flex", alignItems: "center", gap: "3px" }}
