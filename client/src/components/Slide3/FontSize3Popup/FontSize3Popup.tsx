...
const FontSize3Popup = () => {
  ...
  const currentFontSize = selectedTextElement?.fontSize || fontSize3;
  ...
  const updateValidatedSize = (newSize: number) => {
    ...
    if (selectedTextElement) {
      setTextElements3((prev) =>
        prev.map((text) =>
          text.id === selectedTextId3 ? { ...text, fontSize: clampedSize } : text
        )
      );
    } else {
      // Update global context for new text elements
      setFontSize3(clampedSize);
    }
  };
