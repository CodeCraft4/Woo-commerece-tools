...
interface BgChanger4Props {
    onClose: () => void;
    activeIndex?: number;
}

const BgChanger4 = ({ onClose, activeIndex }: BgChanger4Props) => {
    const { setBgColor4, setBgImage4 } = useSlide4();
    ...
    const handlePickerChangeComplete = (color: ColorResult) => {
        const hex = color.hex;
        setPickerColor(hex);
        setBgImage4(null);   // color overrides image
        setBgColor4(hex);
    };
    ...
    const handleBgImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = () => {
            ...
            setBgColor4(null); // image overrides color
            setBgImage4(url);
        };
