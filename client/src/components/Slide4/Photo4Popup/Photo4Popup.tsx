...
const Photo4Popup = ({ onClose, activeIndex, isAdminEditor = false }: Photo4PopupProps) => {
  const {
    images4,
    setSelectedImage4,
    selectedImg4,
  ...
  const pathForUpload = useCallback(
    (fileName: string) => {
      if (isAdminEditor) return fileName;
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    for (const file of files) {
      ...
      if (isAdminEditor) {
        ...
        setImages4((prev: any[]) => [...prev, { id: imageId, src: url, fileName: objectPath }]);
      } else {
        setImages4((prev: any[]) => [...prev, { id: imageId, src: url, fileName: objectPath }]);
  ...
  const handleDelete = async (id: any) => {
    const imgToDelete: any = images4.find((img: any) => img.id === id);
    ...
    setImages4((prev) => prev.filter((img: any) => img.id !== id));
  ...
  const applyFilter = (filterCss: string) => {
    setDraggableImages4((prev) =>
      prev.map((img) => (img.id === activeFilterImageId4 ? { ...img, filter: filterCss } : img))
    );
  };

  if (imageFilter4) {
    const selectedImage = draggableImages4.find((img) => img.id === activeFilterImageId4);
    const previewImg =
          ...
          <Box
            ...
            onClick={() => {
              setSelectedImage4((prev: any) => {
                ...
                if (!isSelected) setActiveFilterImageId4(id);
            ...
            sx={{
              ...
              border: `2px solid ${selectedImg4?.includes(id) ? "#3a7bd5" : "#bd7082ff"}`,
              borderRadius: 2,
              position: "relative",
              cursor: "pointer",
              overflow: "hidden",
