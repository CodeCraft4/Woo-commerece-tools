...
interface BgChanger2Props {
    onClose: () => void;
    activeIndex?: number;
}

const BgChanger2 = ({ onClose, activeIndex }: BgChanger2Props) => {
    const { setBgColor2, setBgImage2 } = useSlide2();
    ...
    const handlePickerChangeComplete = (color: ColorResult) => {
        const hex = color.hex;
        setPickerColor(hex);
        setBgImage2(null);   // color overrides image
        setBgColor2(hex);
    };
    ...
    const handleBgImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = () => {
            ...
            setBgColor2(null); // image overrides color
            setBgImage2(url);
        };
